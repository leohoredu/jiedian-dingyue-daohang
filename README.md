...
> 聚合免费节点、客户端专题、订阅格式检测和连接排查的轻量导航仓库。
...
请打开上面的博客文章，在正文“免费订阅地址”区域获取当天可用入口。导航仓库只做站内入口聚合，正文负责真实订阅和更新状态。
...
本站每日北京时间 00:00 自动处理免费节点用户：重置流量、刷新订阅 Token、写入服务端动态数据，并同步 GitHub 分享仓库。不确定用哪个客户端时，先从专题页和格式检测工具开始。 仓库里的 `sub/latest.txt` 只保存当天文章链接，不保存真实订阅地址。
